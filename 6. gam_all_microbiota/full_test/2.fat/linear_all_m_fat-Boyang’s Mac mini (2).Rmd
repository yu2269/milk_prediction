...
gam_formula_yield = function(variables_names, productions){
  ...
  d_which =which(variables_names == "DIM")
  a_which =which(variables_names == "A.P.Ratio.")
  if (productions == "Milk_lactose") {
    
  }else if (productions == "Milk_yield"){
    df_parity = 3
    df_dim = 2
    df_ap = 2
  ...
  all_which = which(c(p_which, d_which, a_which)!=0)
  
  if (length(all_which)!=0) {
    if (length(variables_names[-c(p_which, d_which, a_which)[all_which]]) == 0 ) {
      variables = c()
      if (length(p_which) != 0 ) {
      ...
      if (length(d_which) != 0 ) {
        variables = append(variables, paste("ns(DIM, df=",df_dim,")",sep = "" ))
      }
      if (length(a_which) != 0 ) {
        # paste("ns(A.P.Ratio., df=",df_dim,")",sep = "" )
        variables = append(variables, paste("ns(A.P.Ratio., df=",df_ap,")",sep = "" ))
      }      
      formula_default = paste(variables, collapse=" + ")
    }else{
      formula_default = paste(variables_names[-c(p_which, d_which, a_which)[all_which]], collapse=" + ")
      if (length(p_which) != 0 ) {
        formula_default = paste(paste("ns(Parity, df=",df_parity,")+",sep = "" ), formula_default)
      }
      if (length(d_which) != 0 ) {
        formula_default = paste(paste("ns(DIM, df=",df_dim,")+",sep = "" ), formula_default)
      }
      if (length(a_which) != 0 ) {
        formula_default = paste(paste("ns(A.P.Ratio., df=",df_ap,")+",sep = "" ), formula_default)
      }
    }}else{
