...
for (i_seed in c(i_min:i_max)) {
  ...
  ## 3.4 build formula
  Formula_bic = formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(regsubsets_fw_bic, collapse=" + ")))
  Formula_bic
  Formula_adjr2 = formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(regsubsets_fw_adjr2, collapse=" + ")))
  Formula_adjr2
  Formula_cp = formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(regsubsets_fw_cp, collapse=" + ")))
  Formula_cp
  ### Record formula
  seed_fold_result[[3]] = Formula_bic
  seed_fold_result[[4]] = Formula_adjr2
  seed_fold_result[[5]] = Formula_cp  
  ## 3.5 use lm refit formula
  lm_bic = lm(formula = Formula_bic, data = train)
  lm_adjr2 = lm(formula = Formula_adjr2, data = train)
  lm_cp = lm(formula = Formula_cp, data = train)
...
for (i_seed in c(i_min:i_max)) {
  ...
  # Formula_glmnet
  Formula_glmnet = FormulaNew = formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(glmnet_variables, collapse=" + ")))
  FormulaNew         
  seed_fold_result[[4]] = Formula_glmnet
  glmModel = lm(formula = Formula_glmnet,data = train)
...
for (i_seed in c(i_min:i_max)) {
  ...
  Formula_rdf_boosting<- formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(rdf_boosting_variables, collapse=" + ")))
  ...
  glmModel = lm(formula = Formula_rdf_boosting,data = train)
  summary_train_model = summary(glmModel)
  summary_train_coef = summary_train_model$coefficients
  #View(summary_train_coef)
  seed_fold_result[[5]] = summary_train_coef
...
for (i_seed in c(i_min:i_max)) {
  ...
  FormulaSCAD = formula(paste(paste(names(train)[1]," ~ ",sep = ""),paste(scadNames, collapse=" + ")))
  FormulaSCAD
  # Need to record this
  seed_fold_result[[3]] = scadNames
  seed_fold_result[[4]] = FormulaSCAD
  glmModel = lm(formula = FormulaSCAD,data = train)
  summary_train_model = summary(glmModel)
  summary_train_coef = summary_train_model$coefficients
  #View(summary_train_coef)
  seed_fold_result[[5]] = summary_train_coef
